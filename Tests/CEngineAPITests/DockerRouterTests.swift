...
private actor FinalPersistenceGate {
    private var armed = false
    private var successfulSavesBeforeBlock = 0
    private var blocked = false
    private var continuation: CheckedContinuation<Void, Never>?

    func arm(afterSuccessfulSaves: Int) {
        armed = true
        successfulSavesBeforeBlock = afterSuccessfulSaves
    }

    func blockWhenArmed() async {
        guard armed else { return }
        guard successfulSavesBeforeBlock == 0 else {
            successfulSavesBeforeBlock -= 1
            return
        }
        armed = false
        blocked = true
...
private actor AutoRemoveCleanupBackend: ContainerBackend {
    ...
    func start(_ container: ContainerRecord) async throws -> [PortBinding] {
        starts += 1
        backendResources.insert(container.id)
        return container.ports
    }
    func stop(_ container: ContainerRecord, timeoutSeconds _: Int) async throws -> Int32 {
        stops += 1
    ...
    func complete(_ identifier: String, code: Int32) {
        completionContinuations.removeValue(forKey: identifier)?.resume(returning: code)
    }
...
private actor PolicyPreparationFailureBackend: ContainerBackend {
    ...
    func prepare(_ container: ContainerRecord) async throws {
        ...
        if failNextPreparation {
            ...
            throw Failure.partialPreparation
        }
    }
    func start(_ container: ContainerRecord) async throws -> [PortBinding] {
        starts += 1
    ...
    func failNextPolicyPreparation(rejectingCleanup: Bool = false) {
        failNextPreparation = true
        rejectCleanup = rejectingCleanup
    }
    func allowCleanup() { rejectCleanup = false }
...
private actor PolicyLaunchQuarantineBackend: ContainerBackend {
    ...
    private var runningContainers = Set<String>()
    private var completionContinuations: [String: CheckedContinuation<Int32?, Never>] = [:]
    private var failNextPolicyStart = false
    private var rejectCleanup = false
    private var starts = 0
    private var stops = 0
    private var deletes = 0
    ...
    func start(_ container: ContainerRecord) async throws -> [PortBinding] {
        starts += 1
        runningContainers.insert(container.id)
        if failNextPolicyStart {
            failNextPolicyStart = false
            rejectCleanup = true
            throw Failure.partialStart
        }
        return container.ports
    ...
    func failNextPolicyLaunchAndCleanup() { failNextPolicyStart = true }
    func allowCleanup() { rejectCleanup = false }
    func complete(_ identifier: String, code: Int32) {
        runningContainers.remove(identifier)
        completionContinuations.removeValue(forKey: identifier)?.resume(returning: code)
    }
    func isWaitingForCompletion(_ identifier: String) -> Bool {
        completionContinuations[identifier] != nil
...
private actor RestartCompletionRaceBackend: ContainerBackend {
    ...
    func restart(_: ContainerRecord, timeoutSeconds _: Int) async throws {
        ...
        throw Failure.restartAfterCompletion
    }
    func completion(_ container: ContainerRecord) async -> Int32? {
        await withCheckedContinuation { completionContinuations[container.id] = $0 }
    }

    func isRestartBlocked() -> Bool { restartBlocked }
    func isWaitingForCompletion(_ identifier: String) -> Bool {
        completionContinuations[identifier] != nil
    }
    func complete(_ identifier: String, code: Int32) {
...
@Suite struct DockerRouterTests {
    ...
    @Test func failedStartCleanupStaysQuarantinedUntilReloadVerifiesTeardown() async throws {
        ...
        // The pre-launch marker save succeeds. Persistence then fails during
        // compensation, proving recovery safety does not depend on a late save.
        await saveFailure.arm(afterSuccessfulSaves: 1, failures: 2)
        ...
        let durableFailure = try await AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
            .load(default: EngineSnapshot())
        #expect(durableFailure.containers.first(where: { $0.id == record.id })?.phase == .created)
        ...
        await runtime.shutdown()
        let quarantinedReload = try await EngineRuntime(root: root, backend: backend)
        #expect(try await quarantinedReload.container(record.id).phase == .dead)
        #expect(await backend.isRunning(record.id))
        await quarantinedReload.shutdown()
    ...
    @Test func removingCleanupPendingContainerClearsItsDurableMarker() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let backend = QuarantinedExecutionBackend()
        let runtime = try await EngineRuntime(root: root, backend: backend)
        let record = try await runtime.createContainer(
            ContainerRecord(name: "remove-cleanup-pending", image: "debian")
        )
        await backend.failStartAndCleanup()
        await #expect(throws: EngineError.self) {
            try await runtime.startContainer(record.id)
        ...
        await runtime.shutdown()
        let reloaded = try await EngineRuntime(root: root, backend: backend)
        await #expect(throws: EngineError.self) { try await reloaded.container(record.id) }
        await reloaded.shutdown()
    }
    ...
    @Test func startupRestartPreparationFailureImmediatelyCleansBackendGeneration() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let backend = PolicyPreparationFailureBackend()
        var record = ContainerRecord(name: "startup-prepare-cleanup", image: "debian")
        record.phase = .exited
        record.startedAt = Date(timeIntervalSince1970: 10)
        record.finishedAt = Date(timeIntervalSince1970: 20)
        record.exitCode = 1
        record.restartPolicy = .init(name: "always")
        try await AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
            .save(EngineSnapshot(containers: [record]))
        await backend.failNextPolicyPreparation()
    ...
    @Test func startupRestartDoesNotPrepareBeforeCleanupFenceIsDurable() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let backend = PolicyPreparationFailureBackend()
        let saveFailure = ArmedStateSaveFailure()
        var record = ContainerRecord(name: "startup-prepare-fence", image: "debian")
        record.phase = .exited
        record.startedAt = Date(timeIntervalSince1970: 10)
        record.finishedAt = Date(timeIntervalSince1970: 20)
        record.exitCode = 1
    ...
    @Test func startupRestartPreparationCleanupFailurePersistsQuarantine() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let backend = PolicyPreparationFailureBackend()
        var record = ContainerRecord(name: "startup-prepare-quarantine", image: "debian")
        record.phase = .exited
        record.startedAt = Date(timeIntervalSince1970: 10)
        record.finishedAt = Date(timeIntervalSince1970: 20)
        record.exitCode = 1
        record.restartPolicy = .init(name: "always")
        var healthy = ContainerRecord(name: "startup-prepare-unrelated", image: "debian")
        healthy.phase = .exited
        healthy.exitCode = 0
        try await AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
            .save(EngineSnapshot(containers: [record, healthy]))
        await backend.failNextPolicyPreparation(rejectingCleanup: true)

        let quarantinedRuntime = try await EngineRuntime(root: root, backend: backend)

        #expect(try await quarantinedRuntime.container(record.id).phase == .dead)
        #expect(try await quarantinedRuntime.container(healthy.id).phase == .exited)
        #expect(await backend.hasBackendResource(record.id))
        let quarantined = try await AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
            .load(default: EngineSnapshot())
        ...
        #expect(quarantined.cleanupPendingContainerIDs?.contains(record.id) == true)
        await quarantinedRuntime.shutdown()

        await backend.allowCleanup()
        let recovered = try await EngineRuntime(root: root, backend: backend)
        #expect(await backend.hasBackendResource(record.id))
        let restarted = try await recovered.container(record.id)
        #expect(restarted.phase == .running)
        #expect(restarted.restartCount == 1)
        let counts = await backend.counts()
    ...
    @Test func startupRestartPolicyPersistsIntentAndQuarantinesUnverifiedPartialLaunchCleanup() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let backend = PolicyLaunchQuarantineBackend()
        var record = ContainerRecord(name: "startup-policy-quarantine", image: "debian")
        record.phase = .exited
        record.startedAt = Date(timeIntervalSince1970: 10)
        record.finishedAt = Date(timeIntervalSince1970: 20)
        record.exitCode = 1
        ...
        try await AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
            .save(EngineSnapshot(containers: [record]))
        await backend.failNextPolicyLaunchAndCleanup()
        ...
        await quarantinedRuntime.shutdown()
        // A later startup retries teardown without relaunching the quarantined
        // record, while the rest of daemon recovery remains available.
        let retryingRuntime = try await EngineRuntime(root: root, backend: backend)
        #expect(try await retryingRuntime.container(record.id).phase == .dead)
        #expect(await backend.counts().starts == 1)
        #expect(await backend.isRunning(record.id))
        await retryingRuntime.shutdown()
    ...
    @Test func cleanupPendingRecoveryRejectsMissingRecordsAndDoesNotDoubleDeleteDeadRecords() async throws {
        ...
        try await AtomicStore<EngineSnapshot>(url: missingRoot.appending(path: "engine.json"))
            .save(EngineSnapshot(cleanupPendingContainerIDs: ["missing-container-id"]))
        await #expect(throws: EngineError.self) {
            _ = try await EngineRuntime(root: missingRoot, backend: MetadataOnlyBackend())
        }
        let missingDurable = try await AtomicStore<EngineSnapshot>(url: missingRoot.appending(path: "engine.json"))
            .load(default: EngineSnapshot())
        #expect(missingDurable.cleanupPendingContainerIDs == ["missing-container-id"])

        let deadRoot = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: deadRoot) }
        let backend = QuarantinedExecutionBackend()
        var dead = ContainerRecord(name: "pending-dead", image: "debian")
        dead.phase = .dead
        try await AtomicStore<EngineSnapshot>(url: deadRoot.appending(path: "engine.json"))
            .save(EngineSnapshot(containers: [dead], cleanupPendingContainerIDs: [dead.id]))
    ...
    @Test func failedLiveAutoRemoveCleanupStaysQuarantinedUntilReloadVerifiesDeletion() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let backend = AutoRemoveCleanupBackend()
        let runtime = try await EngineRuntime(root: root, backend: backend)
        var input = ContainerRecord(name: "auto-remove-cleanup-quarantine", image: "debian")
        ...
        let store = AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
        var publishedFailure: EngineSnapshot?
        let publicationDeadline = Date().addingTimeInterval(2)
        while Date() < publicationDeadline {
            if let candidate = try? await store.load(default: EngineSnapshot()),
               candidate.containers.first(where: { $0.id == record.id })?.phase == .dead,
               candidate.cleanupPendingContainerIDs?.contains(record.id) == true {
                publishedFailure = candidate
                break
            }
            try await Task.sleep(for: .milliseconds(1))
        }

        let durableFailure = try #require(publishedFailure)
        let quarantined = try await runtime.container(record.id)
        #expect(quarantined.phase == .dead)
        ...
        let failedHistory = await runtime.events(since: Date().addingTimeInterval(-60), until: Date())
        var failedHistoryIterator = failedHistory.makeAsyncIterator()
        var failedActions: [String] = []
        while let event = await failedHistoryIterator.next() {
            if event.id == record.id { failedActions.append(event.action) }
        }
        #expect(!failedActions.contains("restart"))
        #expect(!failedActions.contains("destroy"))
        ...
        #expect(refusedCounts.starts == 1)
        #expect(refusedCounts.deletes == 2)
        ...
        #expect(!(await backend.hasBackendResource(record.id)))
        let recoveredCounts = await backend.counts()
        #expect(recoveredCounts.starts == 1)
        // Pending recovery verifies deletion once; auto-remove record cleanup
        // reuses that proof instead of issuing a second delete.
        #expect(recoveredCounts.deletes == refusedCounts.deletes + 1)
        let durable = try await AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
            .load(default: EngineSnapshot())
        #expect(!durable.containers.contains(where: { $0.id == record.id }))
        #expect(durable.cleanupPendingContainerIDs == nil)

        let recoveredHistory = await recovered.events(since: Date().addingTimeInterval(-60), until: Date())
        var recoveredHistoryIterator = recoveredHistory.makeAsyncIterator()
        var recoveredActions: [String] = []
        while let event = await recoveredHistoryIterator.next() {
            if event.id == record.id { recoveredActions.append(event.action) }
        }
        #expect(!recoveredActions.contains("restart"))
        #expect(!recoveredActions.contains("destroy"))
        await recovered.shutdown()
    }
    ...
    @Test func liveAutoRemoveDoesNotDeleteBeforeCleanupMarkerIsDurable() async throws {
        ...
        let recoveredDurable = try await AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
            .load(default: EngineSnapshot())
        #expect(!recoveredDurable.containers.contains(where: { $0.id == record.id }))
        #expect(recoveredDurable.cleanupPendingContainerIDs == nil)
        await recovered.shutdown()
    }

    @Test func recoveredAutoRemoveCompletionWinsOverRestartPolicy() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let backend = AutoRemoveCleanupBackend()
        var record = ContainerRecord(name: "recovered-auto-remove", image: "debian")
        record.phase = .running
        record.startedAt = Date(timeIntervalSince1970: 10)
        record.autoRemove = true
        record.restartPolicy = .init(name: "always")
    ...
    @Test func liveRestartPolicyQuarantinesPartialLaunchUntilReloadCleanup() async throws {
        ...
        let backend = PolicyLaunchQuarantineBackend()
        let runtime = try await EngineRuntime(root: root, backend: backend)
        var input = ContainerRecord(name: "live-policy-quarantine", image: "debian")
        input.restartPolicy = .init(name: "always")
        let record = try await runtime.createContainer(input)
        try await runtime.startContainer(record.id)
        while !(await backend.isWaitingForCompletion(record.id)) { await Task.yield() }
        await backend.failNextPolicyLaunchAndCleanup()
        await backend.complete(record.id, code: 9)
        let store = AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
        var publishedFailure: EngineSnapshot?
        let publicationDeadline = Date().addingTimeInterval(2)
        while Date() < publicationDeadline {
        ...
        #expect(restarted.phase == .running)
        #expect(restarted.restartCount == 1)
        #expect(await backend.counts().starts == 3)
        #expect(await backend.isRunning(record.id))
        await recovered.shutdown()
    }

    @Test func failedExplicitRestartPreservesOldGenerationCompletion() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let backend = RestartCompletionRaceBackend()
        let runtime = try await EngineRuntime(root: root, backend: backend)
        let record = try await runtime.createContainer(
            ContainerRecord(name: "restart-completion-race", image: "debian")
        )
        try await runtime.startContainer(record.id)
        while !(await backend.isWaitingForCompletion(record.id)) { await Task.yield() }
        let wait = try await runtime.subscribeContainerWait(record.id, condition: "next-exit")
        ...
        await backend.releaseRestart()
        await #expect(throws: RestartCompletionRaceBackend.Failure.self) {
            try await restart.value
        }

        let completed = try await runtime.container(record.id)
        #expect(completed.phase == .exited)
        #expect(completed.exitCode == 42)
        #expect(await waitIterator.next() == 42)
        let durable = try await AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
            .load(default: EngineSnapshot())
        #expect(durable.containers.first(where: { $0.id == record.id })?.exitCode == 42)
        #expect(durable.cleanupPendingContainerIDs == nil)
    ...
    @Test func explicitPreparationFailureCleansBackendBeforeSafeRetry() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    @Test func forceRemovalStopFailureStaysQuarantinedAndCannotRestartAcrossReload() async throws {
        ...
        let runtime = try await EngineRuntime(root: root, backend: backend)
        let router = DockerRouter(runtime: runtime, root: root)
        var input = ContainerRecord(name: "force-removal-stop-failure", image: "debian")
        input.restartPolicy = .init(name: "always")
        let record = try await runtime.createContainer(input)
        try await runtime.startContainer(record.id)
        await backend.blockAndFailNextStopAfterSideEffect()
        ...
        let removal = Task {
            await router.route(.init(
                method: .DELETE,
                uri: "/v1.44/containers/\(record.id)?force=1"
            ))
        }
        while !(await backend.isStopBlocked()) { await Task.yield() }
        ...
        let quarantined = try await runtime.container(record.id)
        #expect(quarantined.phase == .dead)
        #expect(quarantined.exitCode == nil)
        #expect(!(await backend.isRunning(record.id)))
        let durableWhileBlocked = try await AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
            .load(default: EngineSnapshot())
        #expect(durableWhileBlocked.containers.first(where: { $0.id == record.id })?.phase == .dead)
        #expect(durableWhileBlocked.cleanupPendingContainerIDs?.contains(record.id) == true)
        #expect(durableWhileBlocked.removalPendingContainerIDs?.contains(record.id) == true)

        await backend.releaseStop()
        #expect(await removal.value.status == .internalServerError)
        let failedCounts = await backend.counts()
        #expect(failedCounts.starts == 1)
        #expect(failedCounts.stops == 1)
        ...
        let recoveredCounts = await backend.counts()
        #expect(recoveredCounts.starts == 1)
        #expect(recoveredCounts.stops == 2)
        #expect(recoveredCounts.deletes == 1)
        #expect(!(await backend.isRunning(record.id)))
        #expect(!(await backend.isPrepared(record.id)))
        let durable = try await AtomicStore<EngineSnapshot>(url: root.appending(path: "engine.json"))
            .load(default: EngineSnapshot())
    ...
    @Test func finalRemovalSaveReservesContainerAndVolumeIdentitiesWithoutLosingConcurrentCreates() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: root) }
        let persistenceGate = FinalPersistenceGate()
        let runtime = try await EngineRuntime(
        ...
        let volume = try await runtime.createVolume(name: "reserved-removal-volume", anonymous: true)
        var input = ContainerRecord(
            id: "reserved-removal-container-id",
            name: "reserved-removal-container",
            image: "debian"
        )
        input.mounts = [.init(kind: .volume, source: volume.name, destination: "/data")]
        let removed = try await runtime.createContainer(input)
