...
| Process security state | Partial | `RTM-001` compares capability masks, checks cengine's default `NoNewPrivs` policy, verifies privileged-exec override, and rejects leaked stage descriptors. `RTM-006` verifies Docker-default capability policy plus `CapAdd`/`CapDrop` parity for init and exec. `RTM-014` verifies OCI-style process rlimits for init, exec, and healthchecks across recovery and restart. `RTM-018` verifies OCI masked/read-only path policy across restart and recovery. `RTM-021` verifies explicit Docker no-new-privileges policy for privileged init, exec, and healthchecks across restart and recovery. | Ambient capabilities, seccomp, AppArmor/SELinux-shaped profiles, and device policy remain intentional gaps and are explicitly rejected when requested. |
| Linux resources and cgroup v2 | Partial | `CTR-028`, `CTR-042`, and `CTR-047` cover CPU/memory limits and live updates; `RTM-004` covers persisted create/update PID limits, enforcement, and recovery; `RTM-008` places each final exec target in its own leaf beneath the container workload cgroup while staging processes remain outside its PID budget, and uses recursive cgroup kill for timed-out healthchecks; `RTM-015` covers persisted `/dev/vda` read/write BPS and IOPS limits, live replace/clear, direct-I/O enforcement, transactional failure behavior, and recovery through cgroup-v2 `io.max`; `RTM-019` records block-I/O weights as an architecture gap and verifies versioned fail-closed behavior. | Docker-relative block-I/O weights cannot coordinate separate VM kernels and disks. Non-root devices, device access controls, and broader cgroup-v2 delegation/accounting remain gaps. |
| Linux devices and filesystems | Partial | `CTR-035`, `CTR-045`, `VOL-002`–`VOL-006`, `RTM-002`, `RTM-020`, `RTM-022`, and `RTM-023` cover ext4, standard devices, volumes, tmpfs size/mode/execution policy, private bind mounts, bind recursion/read-only controls, and nested named-volume write policy; `NET-019` covers endpoint-scoped network sysctls; `RTM-013` covers fail-closed structured mount options without anonymous-volume leakage. | Configurable devices, device cgroup rules, container-wide sysctls, shared/slave propagation, and additional filesystem options remain intentional gaps and are explicitly rejected when requested. |
