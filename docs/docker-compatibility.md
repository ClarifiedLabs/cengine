...
| Runtime semantics | `RTM-001`–`RTM-007`, `KND-001` | PID limits, bind propagation, configurable capability add/drop, and filter-safe container pruning are covered. Configurable devices/rlimits/seccomp/security options, masked paths, and broader cgroup-v2 IO/device behavior remain gaps with explicit rejection where decoded. |
...
| Read-only root and writable mounts | Covered | `RTM-002` requires exec writes to fail on the root while an explicit tmpfs remains writable; `RTM-005` verifies a requested recursive-shared bind in inspect and Linux mountinfo. | Expand bind/volume remount and propagation matrices beyond the focused recursive-shared contract. |
...
| Linux resources and cgroup v2 | Partial | `CTR-028`, `CTR-042`, and `CTR-047` cover CPU/memory limits and live updates; `RTM-004` covers persisted create/update PID limits, enforcement, and recovery; final exec targets join the container workload cgroup while staging processes remain outside its PID budget. | IO/device controls, per-device blkio, and broader cgroup-v2 delegation/accounting remain gaps. |
| Linux devices and filesystems | Partial | `CTR-035`, `CTR-045`, `VOL-002`–`VOL-006`, `RTM-002`, and `RTM-005` cover ext4, standard devices, volumes, tmpfs, and bind propagation. | Configurable devices, device cgroup rules, sysctls, non-recursive bind variants, and additional filesystem options remain intentional gaps and are explicitly rejected when requested. |
...
| `RTM-005` | `test_bind_mount_propagation_is_applied_and_inspected` | ✅ Pass | Support | **cengine-owned.** Docker bind propagation is validated, persisted, inspected, carried over the guest protocol, and realized with Linux mount propagation flags; the focused black-box contract verifies `rshared` in mountinfo. Non-recursive bind variants are explicitly unsupported. See [Docker bind propagation](https://docs.docker.com/engine/storage/bind-mounts/#configure-bind-propagation) and [`mount(2)` shared-subtree operations](https://man7.org/linux/man-pages/man2/mount.2.html). |
| `RTM-006` | `test_capability_add_drop_apply_to_init_and_exec` | ✅ Pass | Support | **cengine-owned.** Docker's default Linux capability set is used for unprivileged root workloads; case-insensitive `CapAdd`/`CapDrop`, optional `CAP_` prefixes, and `ALL` are normalized with Docker's combined add/drop semantics and applied to bounding/permitted/effective sets for init and default exec. Privileged workloads and privileged exec retain all capabilities. See [Docker Linux capabilities](https://docs.docker.com/engine/containers/run/#runtime-privilege-and-linux-capabilities) and [OCI process capabilities](https://github.com/opencontainers/runtime-spec/blob/v1.3.0/config.md#process). |
| `RTM-007` | `test_container_prune_honors_filters_and_rejects_unknown_keys` | ✅ Pass | Support | **cengine-owned.** Container prune applies Docker `until`, positive-label, and negated-label filters before deletion. Unknown or malformed filters fail without deleting any container instead of silently broadening prune scope. See the [Docker container prune API](https://docs.docker.com/reference/api/engine/version/v1.52/#tag/Container/operation/ContainerPrune). |
...
| `Mount.BindOptions.NonRecursive`, `ReadOnlyNonRecursive`, `ReadOnlyForceRecursive` | Intentional gap | Enabled requests return HTTP 501; recursive bind propagation remains supported. |
