...
| Runtime semantics | `RTM-001`–`RTM-024`, `KND-001` | PID limits, process rlimits, per-root-device block-I/O throttles, default cgroup-v2 device access, private bind isolation, bind recursion/read-only controls, structured tmpfs execution flags, named-volume/rootfs read-only composition, configurable capability add/drop, explicit no-new-privileges policy, masked/read-only paths, filter-safe container pruning, paused lifecycle control, attached/detached exec status behavior, independent IPv4/IPv6 default-route selection, IPC `none`, durable namespace-mode inspect, and fail-closed API v1.55 runtime-input handling are covered. Docker-host/cross-container namespace sharing, shared/slave bind propagation, and blkio weights have explicit architecture decisions; configurable devices, custom device rules, seccomp/AppArmor profiles, and broader cgroup-v2 IO accounting remain intentional or undecided gaps. |
...
| Read-only root and writable mounts | Covered | `RTM-002` requires exec writes to fail on the root while an explicit tmpfs remains writable; private and recursive-private bind modes are applied. `RTM-020` applies Docker's recursive/non-recursive bind and read-only selections across restart and recovery. `RTM-022` applies Docker's default tmpfs `noexec` policy and explicit structured `exec`/`noexec` overrides. `RTM-023` composes a read-only root with nested read-only and writable block-backed and shared named volumes, including restart and recovery. | Shared/slave propagation cannot cross the macOS virtiofs boundary and is explicitly rejected by `RTM-005`; accepted root, bind, volume, and tmpfs write-policy combinations now have focused coverage. |
| PID, mount, UTS, IPC, network and cgroup namespaces | Partial | `RTM-001` requires init/default exec to share all six private namespace identities; `RTM-012` requires independent IPv4 and IPv6 default-route selection across mixed-family endpoints. `RTM-016` requires accepted namespace selections to persist, inspect, and survive recovery, and applies IPC `none` as a private IPC namespace without `/dev/shm`. `RTM-017` fails closed on every Docker-host and cross-container sharing mode before mutation. | Docker-host and cross-container sharing cannot cross separate per-container VM kernels and is an explicit architecture gap. OCI namespace paths are not exposed because cengine has neither a Docker input for them nor an OCI runtime CLI; reassess only if the architecture or exposed surface changes. |
| Process security state | Partial | `RTM-001` compares capability masks, checks cengine's default `NoNewPrivs` policy, verifies privileged-exec override, and rejects leaked stage descriptors. `RTM-006` verifies Docker-default capability policy plus `CapAdd`/`CapDrop` parity for init and exec. `RTM-014` verifies OCI-style process rlimits for init, exec, and healthchecks across recovery and restart. `RTM-018` verifies OCI masked/read-only path policy across restart and recovery. `RTM-021` verifies explicit Docker no-new-privileges policy for privileged init, exec, and healthchecks across restart and recovery. `RTM-024` denies unprivileged access to VM disks through the workload cgroup while retaining Docker/runc's standard device policy. | Ambient capabilities and seccomp or AppArmor/SELinux-shaped profiles remain intentional gaps. Configurable device additions and custom device-policy rules remain explicitly rejected. |
| Linux resources and cgroup v2 | Partial | `CTR-028`, `CTR-042`, and `CTR-047` cover CPU/memory limits and live updates; `RTM-004` covers persisted create/update PID limits, enforcement, and recovery; `RTM-008` places each final exec target in its own leaf beneath the container workload cgroup while staging processes remain outside its PID budget, and uses recursive cgroup kill for timed-out healthchecks; `RTM-015` covers persisted `/dev/vda` read/write BPS and IOPS limits, live replace/clear, direct-I/O enforcement, transactional failure behavior, and recovery through cgroup-v2 `io.max`; `RTM-019` records block-I/O weights as an architecture gap and verifies versioned fail-closed behavior; `RTM-024` attaches a `BPF_PROG_TYPE_CGROUP_DEVICE` allowlist before workload placement and relies on hierarchical inheritance for exec leaves. | Docker-relative block-I/O weights cannot coordinate separate VM kernels and disks. Non-root configured devices, custom device rules, and broader cgroup-v2 delegation/accounting remain gaps. |
| Linux devices and filesystems | Partial | `CTR-035`, `CTR-045`, `VOL-002`–`VOL-006`, `RTM-002`, `RTM-020`, `RTM-022`, `RTM-023`, and `RTM-024` cover ext4, standard device creation and access control, volumes, tmpfs size/mode/execution policy, private bind mounts, bind recursion/read-only controls, and nested named-volume write policy; `NET-019` covers endpoint-scoped network sysctls; `RTM-013` covers fail-closed structured mount options without anonymous-volume leakage. | Configurable devices, custom device cgroup rules, container-wide sysctls, shared/slave propagation, and additional filesystem options remain intentional gaps and are explicitly rejected when requested. |
...
| 1.55 | Image attestations and per-device blkio updates | Partial | Attached in-toto statements support platform/type filters and statement opt-in. The four throttle arrays support the VM root disk `/dev/vda`: create applies them across API v1.44–v1.55, while v1.55 update treats omitted or null fields as unchanged and empty arrays as independent clears (reported as `max` by cgroup-v2 `io.max`); earlier update APIs strip these fields before typed decoding and ignore even malformed values (`RTM-015`). `BlkioWeightDevice` update has the same pre-v1.55 ignore behavior, but active create and v1.55 update requests return HTTP 501 because relative I/O scheduling cannot cross cengine's per-container VMs (`RTM-019`). |
...
| `RTM-015` | `test_block_io_throttles_apply_update_enforce_and_survive_recovery` | ✅ Pass | Support | **cengine-owned.** Docker's four per-device BPS/IOPS throttle arrays persist, inspect, and apply to the VM root disk `/dev/vda`. The guest resolves the trusted host device to `major:minor` and programs cgroup-v2 `io.max` `rbps`, `wbps`, `riops`, and `wiops`; a direct-I/O arm64 BusyBox probe checks kernel enforcement. API v1.55 live updates independently replace or clear fields, with omitted/null fields unchanged; kernel readback represents a cleared limit as `max`. API v1.44–v1.54 strip the four fields before typed decoding and ignore malformed values. Resource replacement is transactional across scalar and block-I/O writes when restoration succeeds; termination, candidate-launch, or restoration failures attempt cleanup and restoration, and remain rollback-incomplete unless restoration is proven. Raw shim ownership is durable per generation: an immutable spec and pre-spawn intent exist before process creation, the child publishes its exact PID/start identity as its first action, and recovery can adopt the unique intent from process argv if the parent dies immediately after spawn. Startup, retry, orphan cleanup, and deletion enumerate every generation independently; PID reuse is rejected, and writable roots remain while any termination is unproven. Before touching the backend, the host durably journals complete old and desired records plus the transaction phase. All snapshot writes share one serialized order, and only a single atomic desired-record/journal-removal save publishes success. A failed save compensates the guest back to the old resources or fails closed through teardown; daemon recovery reapplies and verifies the durable old selection before accepting a live or stopped execution. Stopped recovery seeds the exact original container record and VM capacity from persisted preparation state, reconstructs a missing shim from that state and writable root, and failed replacement restores the original shim without deleting that root. An unresolved journal reserves the stable container ID/name/creation identity and fences start, restart, deferred completion, auto-remove, and non-definitive removal; definitive deletion removes the record, fences, and journal in one durable transition. A compatibility-only marker targets one container and is claimed by no-overwrite rename before validation, then only that claimed directory entry is consumed; its fault fails a later guest write after memory and `rbps` have changed. The test verifies scalar and `io.max` rollback plus unchanged inspect and persisted records, then applies configured and cleared limits while stopped and reads them back after start, Docker `/restart`, and daemon recovery. Other device paths return HTTP 501; malformed paths, non-positive rates, and per-array duplicates return HTTP 400. See Docker's [API v1.55 update semantics](https://docs.docker.com/reference/api/engine/version-history/#v155-api-changes), the [Linux cgroup-v2 `io.max` contract](https://docs.kernel.org/admin-guide/cgroup-v2.html#io), and [OCI block-I/O resources](https://github.com/opencontainers/runtime-spec/blob/v1.3.0/config-linux.md#block-io). |
| `RTM-016` | `test_ipc_none_omits_shared_memory_and_survives_recovery` | ✅ Pass | Support | **cengine-owned.** Accepted private/default cgroup, IPC, PID, and UTS selections plus host userns persist and inspect. Docker IPC `none` creates a private IPC namespace without mounting `/dev/shm`; the selection and mount state introduced in guest protocol v11 are carried by the current v13 protocol. See Docker's [`--ipc` modes](https://docs.docker.com/reference/cli/docker/container/run/#ipc-settings---ipc) and [OCI Linux namespace creation](https://github.com/opencontainers/runtime-spec/blob/v1.3.0/config-linux.md#namespaces). |
| `RTM-017` | `test_same_kernel_namespace_sharing_fails_closed` | ✅ Pass | Intentional gap | **cengine-owned.** Docker-host and cross-container cgroup, IPC, PID, UTS, and network sharing return HTTP 501 before container or anonymous-volume mutation because separate per-container VM kernels cannot join the same Linux namespace. Arbitrary path-shaped Docker mode values return HTTP 400. OCI namespace paths are not exposed through the Docker API or an OCI runtime CLI. See Docker's [namespace mode semantics](https://docs.docker.com/reference/cli/docker/container/run/) and [OCI namespace paths](https://github.com/opencontainers/runtime-spec/blob/v1.3.0/config-linux.md#namespaces). |
| `RTM-018` | `test_masked_and_readonly_paths_apply_restart_and_survive_recovery` | ✅ Pass | Support | **cengine-owned.** Docker `MaskedPaths` and `ReadonlyPaths` persist, inspect, and enter the guest protocol (introduced in v11 and carried by current v13). Omitted lists select Docker's defaults, explicit empty lists disable them, and privileged workloads clear them; focused Swift coverage verifies those selections. Missing targets are ignored. Existing files are masked by a bind mount from a verified open descriptor for character device `1:3`; directories are covered with an empty read-only tmpfs; and read-only paths retain inherited `nosuid`, `nodev`, and `noexec` flags. Named init identities resolve before masking, and when identity files are masked the mounted passwd/group view is snapshotted for later named exec resolution, including bind-mounted supplementary groups. The policy remains active for exec after container restart and daemon recovery. Relative inputs return HTTP 400. See Moby v29.6.2's [default OCI paths](https://github.com/moby/moby/blob/docker-v29.6.2/daemon/pkg/oci/defaults.go) and [host-config override handling](https://github.com/moby/moby/blob/docker-v29.6.2/daemon/oci_linux.go), OCI Runtime Spec v1.3.0 [masked paths and read-only paths](https://github.com/opencontainers/runtime-spec/blob/v1.3.0/config-linux.md#masked-paths), runc v1.3.3's [`readonlyPath` and `maskPaths`](https://github.com/opencontainers/runc/blob/v1.3.3/libcontainer/rootfs_linux.go), and the descriptor-verification rationale in [GHSA-9493-h29p-rfm2](https://github.com/opencontainers/runc/security/advisories/GHSA-9493-h29p-rfm2). |
| `RTM-019` | `test_block_io_weights_are_an_architecture_gap_with_versioned_update_semantics` | ✅ Pass | Intentional gap | **cengine-owned.** Docker block-I/O weights distribute a shared block device between sibling container cgroups. A cgroup weight inside one cengine guest can compete only with processes in that VM; it cannot arbitrate against containers with separate kernels and virtual disks, and Virtualization.framework exposes no host disk-weight control. Active `BlkioWeight` create/update and `BlkioWeightDevice` create/API v1.55 update requests therefore return HTTP 501 before container, volume, or resource mutation. API v1.44–v1.54 updates strip `BlkioWeightDevice` before typed decoding and preserve the historical ignored behavior; inspect reports the inert Docker defaults. Malformed recognized values return HTTP 400. See Docker's [block-I/O weight contract](https://docs.docker.com/reference/cli/docker/container/run/#block-io-bandwidth-blkio-constraint), Linux's [cgroup-v2 weight distribution model](https://docs.kernel.org/admin-guide/cgroup-v2.html#weights), and Apple's [disk-image storage attachment API](https://developer.apple.com/documentation/virtualization/vzdiskimagestoragedeviceattachment). |
| `RTM-020` | `test_bind_recursion_and_readonly_modes_apply_restart_and_survive_recovery` | ✅ Pass | Support | **cengine-owned.** Structured bind mounts persist Docker's `NonRecursive`, `ReadOnlyNonRecursive`, and `ReadOnlyForceRecursive` selections and apply them through guest protocol v12, carried by current v13. Bind creation uses `MS_BIND` or `MS_BIND|MS_REC`; read-only binds default to recursive `mount_setattr(AT_RECURSIVE, MOUNT_ATTR_RDONLY)`, the non-recursive option remounts only the bind root, and the force option forbids compatibility fallback. The applied read-only/writable state survives container restart and daemon recovery. Contradictory read-only modes return HTTP 400 before container or anonymous-volume mutation. See Moby v29.6.2's [bind option definitions](https://github.com/moby/moby/blob/docker-v29.6.2/api/types/mount/mount.go) and [OCI mount option selection](https://github.com/moby/moby/blob/docker-v29.6.2/daemon/oci_linux.go#L595-L618), plus Linux [`mount_setattr(2)`](https://man7.org/linux/man-pages/man2/mount_setattr.2.html). |
| `RTM-021` | `test_no_new_privileges_security_option_applies_restart_and_survives_recovery` | ✅ Pass | Support | **cengine-owned.** Docker `SecurityOpt` accepts bare and boolean `no-new-privileges` selections, persists and inspects the submitted option list, and applies the selected `PR_SET_NO_NEW_PRIVS` policy to init, default exec, privileged exec, and healthchecks through guest protocol v13. Omitted policy retains cengine's existing unprivileged/privileged defaults; an explicit selection overrides those defaults and survives container restart and daemon recovery. Invalid boolean values return HTTP 400, while unsupported security profiles continue to return HTTP 501. See Docker's [`--security-opt no-new-privileges`](https://docs.docker.com/reference/cli/docker/container/run/#optional-security-options---security-opt), Moby v29.1.5's [security-option parsing and boolean override contract](https://github.com/moby/moby/blob/docker-v29.1.5/daemon/daemon_unix.go), OCI Runtime Spec v1.3.0 [`noNewPrivileges`](https://github.com/opencontainers/runtime-spec/blob/v1.3.0/config.md#posix-process), and Linux [`PR_SET_NO_NEW_PRIVS`](https://man7.org/linux/man-pages/man2/PR_SET_NO_NEW_PRIVS.2const.html). |
| `RTM-022` | `test_structured_tmpfs_execution_options_apply_restart_and_survive_recovery` | ✅ Pass | Support | **cengine-owned.** Structured tmpfs mounts are writable but `noexec,nosuid,nodev` by default. API v1.46's `TmpfsOptions.Options` accepts the Docker/Moby `exec` and `noexec` flags, persists and inspects their submitted order, applies the last execution flag in the guest, and survives container restart and daemon recovery. API v1.45 inspect omits the later `Options` field. Unknown options and non-flag-shaped arrays return HTTP 400 before container or anonymous-volume mutation. See Moby v29.0.0's [tmpfs option API shape](https://github.com/moby/moby/blob/docker-v29.0.0/api/types/mount/mount.go#L116-L151), [accepted execution flags](https://github.com/moby/moby/blob/docker-v29.0.0/daemon/volume/mounts/linux_parser.go#L230-L249), and [Docker default tmpfs flags](https://github.com/moby/moby/blob/docker-v29.0.0/daemon/oci_linux.go#L521-L534), plus Linux [`mount(2)`](https://man7.org/linux/man-pages/man2/mount.2.html). |
| `RTM-023` | `test_volume_readonly_matrix_orders_nested_mounts_and_survives_recovery` | ✅ Pass | Support | **cengine-owned.** User mounts are applied parent-before-child regardless of request order so a parent cannot hide a nested mount. A read-only root remains immutable while explicitly writable named volumes stay writable; read-only volumes are recursively read-only, and a later writable child mount remains writable beneath a read-only parent. The matrix covers both direct ext4 block volumes and storage-appliance NFS volumes for init and exec across container restart and daemon recovery. See Moby v29.6.2's [mount depth ordering and recursive read-only selection](https://github.com/moby/moby/blob/docker-v29.6.2/daemon/oci_linux.go#L479-L638), OCI Runtime Spec v1.3.0 [read-only root](https://github.com/opencontainers/runtime-spec/blob/v1.3.0/config.md#root) and [ordered mount](https://github.com/opencontainers/runtime-spec/blob/v1.3.0/config.md#mounts) requirements, and Linux [`mount_setattr(2)` recursive read-only semantics](https://man7.org/linux/man-pages/man2/mount_setattr.2.html). |
| `RTM-024` | `test_default_device_policy_blocks_vm_disks_and_survives_recovery` | ✅ Pass | Support | **cengine-owned.** Every unprivileged workload cgroup receives Docker/runc's default device allowlist through cgroup-v2 BPF before the workload process is placed. Standard character devices and pseudoterminals retain `rwm` access, while arbitrary character and block nodes retain only Docker's default `mknod` permission; recreating the discoverable `/dev/vda` node therefore succeeds but opening it fails with `EPERM`. Exec leaves inherit the policy, and container restart plus daemon recovery retain enforcement. Transient verifier `EAGAIN` responses are retried as required by `bpf(2)`. Privileged workloads keep their existing unrestricted device surface. Configurable devices and custom `DeviceCgroupRules` remain explicit gaps. See runc v1.3.3's [default allowed-device rules](https://github.com/opencontainers/runc/blob/v1.3.3/libcontainer/specconv/spec_linux.go#L776-L922), OCI Runtime Spec v1.3.0 [default devices and allowed device list](https://github.com/opencontainers/runtime-spec/blob/v1.3.0/config-linux.md#devices), Linux's [cgroup-v2 BPF device controller](https://docs.kernel.org/admin-guide/cgroup-v2.html#device-controller), and [`bpf(2)` `EAGAIN` handling](https://man7.org/linux/man-pages/man2/bpf.2.html#ERRORS). |
...
| `CgroupnsMode`, `IpcMode`, `PidMode`, `UTSMode`, and `UsernsMode` | Partial | Private/default cgroup, IPC, PID, and UTS selections plus host userns are persisted and inspected. IPC `none` creates a private IPC namespace without `/dev/shm` and survives recovery (`RTM-016`). IPC `shareable`, every Docker-host/cross-container sharing mode, and arbitrary namespace-path-shaped mode values fail before mutation (`RTM-017`). |
...
| runtime, process-group, OOM-score, shared-memory, and isolation overrides | Intentional gap | Inert values are accepted. Custom runtime/groups/storage options, non-default OOM score, non-default shared-memory size, and non-default isolation return HTTP 501; invalid values return HTTP 400. |
| `HostConfig.SecurityOpt` | Partial | Bare and boolean `no-new-privileges` selections persist, inspect, and apply to init, exec, and healthchecks across restart and recovery (`RTM-021`). Privileged containers also accept kind's inert `seccomp=unconfined` and `apparmor=unconfined` defaults. Other seccomp and LSM-shaped requests return HTTP 501. |
...
| create/exec `ConsoleSize` | Intentional gap | Container create ignores a well-formed `HostConfig.ConsoleSize` when `Tty=false`, because Docker CLI 29.6.2 sends its own terminal size even for non-TTY creates. With `Tty=true`, and for exec create/start, `[0,0]` is accepted as the inert default and a non-zero terminal size returns HTTP 501. Malformed shapes return HTTP 400 in every case. |
...
| `CTR-015` | `test_copy_to_container` | ✅ Pass | Support | Content, mode, and numeric tar UID/GID are preserved inside the guest; repeated uploads replace regular files and symlinks without following link targets, existing directories merge, and the default archive endpoint replaces directory/non-directory type changes. |
...
| `BLD-005` | `test_buildx_recovers_uplink_after_network_helper_restart` | ✅ Pass | Support | **cengine-owned.** A running BuildKit VM automatically recreates its vmnet uplink after the dedicated compatibility helper performs an authenticated, launchd-managed restart without another administrator session. |
