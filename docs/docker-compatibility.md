...
| `RTM-016` | `test_ipc_none_omits_shared_memory_and_survives_recovery` | ✅ Pass | Support | **cengine-owned.** Accepted private/default cgroup, IPC, PID, and UTS selections plus host userns persist and inspect. Docker IPC `none` creates the workload in its own IPC namespace without mounting `/dev/shm`; the selection and mount state survive daemon recovery through guest protocol v10. See Docker's [`--ipc` modes](https://docs.docker.com/reference/cli/docker/container/run/#ipc-settings---ipc) and [OCI Linux namespace creation](https://github.com/opencontainers/runtime-spec/blob/v1.3.0/config-linux.md#namespaces). |
