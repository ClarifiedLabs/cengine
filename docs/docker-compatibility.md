...
| Runtime semantics | `RTM-001`–`RTM-008`, `KND-001` | PID limits, private bind isolation, configurable capability add/drop, filter-safe container pruning, and exec stage signal/status behavior are covered. Shared/slave bind propagation, configurable devices/rlimits/seccomp/security options, masked paths, and broader cgroup-v2 IO/device behavior remain gaps with explicit rejection where decoded. |
...
| Linux resources and cgroup v2 | Partial | `CTR-028`, `CTR-042`, and `CTR-047` cover CPU/memory limits and live updates; `RTM-004` covers persisted create/update PID limits, enforcement, and recovery; final exec targets join a dedicated leaf beneath the container workload cgroup while staging processes remain outside its PID budget. | IO/device controls, per-device blkio, and broader cgroup-v2 delegation/accounting remain gaps. |
...
| `RTM-008` | `test_exec_stage_proxies_preserve_status_and_kill_timed_out_healthchecks` | ✅ Pass | Support | **cengine-owned.** Exec staging processes preserve target exit codes, proxy catchable signals, and target the final staged child directly for uncatchable signals so timed-out healthchecks cannot survive as live orphan processes. |
