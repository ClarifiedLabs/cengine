...
| Read-only root and writable mounts | Partial | `RTM-002` requires exec writes to fail on the root while an explicit tmpfs remains writable; private and recursive-private bind modes are applied. `RTM-020` applies Docker's recursive/non-recursive bind and read-only selections across restart and recovery. `RTM-022` applies Docker's default tmpfs `noexec` policy and explicit structured `exec`/`noexec` overrides. | Shared/slave propagation cannot cross the macOS virtiofs boundary and is explicitly rejected by `RTM-005`; expand volume remount matrices. |
...
| Linux devices and filesystems | Partial | `CTR-035`, `CTR-045`, `VOL-002`–`VOL-006`, `RTM-002`, `RTM-020`, and `RTM-022` cover ext4, standard devices, volumes, tmpfs size/mode/execution policy, private bind mounts, and bind recursion/read-only controls; `NET-019` covers endpoint-scoped network sysctls; `RTM-013` covers fail-closed structured mount options without anonymous-volume leakage. | Configurable devices, device cgroup rules, container-wide sysctls, shared/slave propagation, and additional filesystem options remain intentional gaps and are explicitly rejected when requested. |
